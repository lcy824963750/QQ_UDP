...
public class ToolsParseMsg {
	...
	public static MsgHead parseMsg(byte[] data) throws Exception {
		...
		if (msgType == IMsgConstance.command_reg) {
			...
			return msgHead;
		} else if(msgType == IMsgConstance.command_msgUpdate) {
			return msgHead;
		} else if(msgType == IMsgConstance.command_sendAddr) {
			return msgHead;
		} else if(msgType == IMsgConstance.command_sendPort) {
			return msgHead;
		} else if(msgType == IMsgConstance.command_msgUpdate_resp) {
			MsgUpdateResp msgUpdateResp = new MsgUpdateResp();
			copyHead(msgHead, msgUpdateResp);
			msgUpdateResp.setState(dins.readByte());
			return msgUpdateResp;
		} else if(msgType == IMsgConstance.command_leave) {
			return msgHead;
...
//			chatFile.setFileData(data1);
			...
			byte[] data1 = new byte[community.getLen()];
			dins.readFully(data1);
			...
			return msgHead;
		} else if(msgType == IMsgConstance.command_confirmPort) {
			return msgHead;
