...
public class CommunityTree extends JTree {
	...
	// 创建树时，传入jkuser对象
	public CommunityTree(Jkuser jkuser, MainUI mainUI) {
		try {
			sender = new UDPSender(mainUI, null);
			...
			public void actionPerformed(ActionEvent e) {
				...
				btn.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						community.setLen(len);
						try {
							sender.sendMsg(community);
						} catch (Exception e1) {
							e1.printStackTrace();
						}
		...
		deleteCommunity.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					sender.sendMsg(deleteCommunity);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
	...
	public void onMsgRecive(MsgHead m) {
		if (m.getType() == IMsgConstance.command_commuChatTxt) {
			if(m.getSrc() == jkuser.getJknum()) return;
	...
	private void showSendMsgUI(final Community destCommunity) {
		...
		send_btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				chatText.setSrc(jkuser.getJknum());
				chatText.setDest(0);
				chatText.setchatTxt(txt);
				chatText.setDestCid(destCommunity.getCid());
				chatText.setType(IMsgConstance.command_commuChatTxt);
				chatText.setSendTime(sendTime);
				sender.sendCastMsg(chatText);
